...
# The production activity may already contain the hardened media assembler.
# In that case do not rewrite it. Otherwise install the canonical AAC+MP4 path.
if 'private fun encodeWavToAacMp4(' not in a:
    block='''    private fun muxVideoAudio(videoUri:android.net.Uri,audio:File,out:File){
        ...
        try{
            ...
            val videoDuration=if(videoFormat.containsKey(MediaFormat.KEY_DURATION))videoFormat.getLong(MediaFormat.KEY_DURATION)else 0L
    ...
    private fun copyTrackBounded(ex:MediaExtractor,mux:MediaMuxer,track:Int,maxDurationUs:Long){
        ...
        while(true){val n=ex.readSampleData(buf,0);if(n<0)break;val pts=ex.sampleTime;if(pts<0||pts>maxDurationUs)break;if(pts<lastPts)throw IllegalStateException("non-monotonic media timestamps");info.offset=0;info.size=n;info.presentationTimeUs=pts;info.flags=ex.sampleFlags;mux.writeSampleData(track,buf,info);lastPts=pts;ex.advance()}
    }
    ...
    private fun readWavInfo(file:File):WavInfo{
        ...
        try{
            ...
            val riff=ByteArray(4);raf.readFully(riff);if(String(riff,Charsets.US_ASCII)!="RIFF")throw IllegalStateException("WAV RIFF header missing");raf.skipBytes(4);val wave=ByteArray(4);raf.readFully(wave);if(String(wave,Charsets.US_ASCII)!="WAVE")throw IllegalStateException("WAV WAVE header missing")
            ...
            while(raf.filePointer+8L<=raf.length()){
                ...
                when(String(id,Charsets.US_ASCII)){"fmt "->{audioFormat=readLeShort(raf);channels=readLeShort(raf);sampleRate=readLeInt(raf);raf.skipBytes(6);bits=readLeShort(raf)};"data"->{dataOffset=raf.filePointer;dataSize=size}}
    ...
    private fun encodeWavToAacMp4(wav:File,out:File){
        ...
        val w=readWavInfo(wav);if(out.exists())out.delete();val codec=MediaCodec.createEncoderByType("audio/mp4a-latm")
        ...
        var mux:MediaMuxer?=null;var muxTrack=-1;var muxStarted=false;var eosQueued=false;var done=false;val raf=java.io.RandomAccessFile(wav,"r");raf.seek(w.dataOffset);var remaining=w.dataSize;var ptsUs=0L
        try{
            codec.configure(fmt,null,null,MediaCodec.CONFIGURE_FLAG_ENCODE);codec.start()
            while(!done){
                if(!eosQueued){val ii=codec.dequeueInputBuffer(10000);if(ii>=0){val input=codec.getInputBuffer(ii)?:throw IllegalStateException("AAC input buffer unavailable");input.clear();val frameBytes=w.channels*2;var toRead=minOf(input.remaining(),remaining.coerceAtMost(Int.MAX_VALUE.toLong()).toInt());toRead-=toRead%frameBytes;if(toRead>0){val bytes=ByteArray(toRead);raf.readFully(bytes);input.put(bytes);val frames=toRead/frameBytes;codec.queueInputBuffer(ii,0,toRead,ptsUs,0);ptsUs+=(frames*1000000L)/w.sampleRate;remaining-=toRead}else{codec.queueInputBuffer(ii,0,0,ptsUs,MediaCodec.BUFFER_FLAG_END_OF_STREAM);eosQueued=true}}}
                val info=MediaCodec.BufferInfo();val oi=codec.dequeueOutputBuffer(info,10000)
                when{oi==MediaCodec.INFO_OUTPUT_FORMAT_CHANGED->{if(muxStarted)throw IllegalStateException("AAC output format changed twice");mux=MediaMuxer(out.absolutePath,MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);muxTrack=mux.addTrack(codec.outputFormat);mux.start();muxStarted=true};oi==MediaCodec.INFO_TRY_AGAIN_LATER->{if(eosQueued)continue};oi>=0->{val buffer=codec.getOutputBuffer(oi);if(buffer!=null&&info.size>0&&(info.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG)==0){if(!muxStarted)throw IllegalStateException("AAC muxer not started");buffer.position(info.offset);buffer.limit(info.offset+info.size);mux!!.writeSampleData(muxTrack,buffer,info)};if((info.flags and MediaCodec.BUFFER_FLAG_END_OF_STREAM)!=0)done=true;codec.releaseOutputBuffer(oi,false)}}
            }
            if(!muxStarted)throw IllegalStateException("AAC encoder produced no output format");mux?.stop()
...
'''
    ...
    print('STAGE 11 V2: AAC/MP4 hardening applied')
else:
    print('STAGE 11 V2: canonical AAC/MP4 hardening already present')

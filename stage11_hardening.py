...
if not activity.is_file():
    raise SystemExit('STAGE 11 HARDENING: activity_fixed.kt missing')
a = activity.read_text(encoding='utf-8')

old = '''    private fun muxVideoAudio(videoUri:android.net.Uri,audio:File,out:File){
...
new = '''    private fun muxVideoAudio(videoUri:android.net.Uri,audio:File,out:File){
        if(out.exists())out.delete()
        val vf=contentResolver.openFileDescriptor(videoUri,"r")?:throw IllegalStateException("video FD unavailable")
        val ve=MediaExtractor()
        val encodedAudio=File(cacheDir,"aac_${sid}.mp4")
        var mux:MediaMuxer?=null
        var ae:MediaExtractor?=null
        try{
            ve.setDataSource(vf.fileDescriptor)
            var videoFormat:MediaFormat?=null;var videoTrack=-1
            for(i in 0 until ve.trackCount){val f=ve.getTrackFormat(i);if(f.getString(MediaFormat.KEY_MIME)?.startsWith("video/")==true){videoFormat=f;videoTrack=i;break}}
            val vfmt=videoFormat?:throw IllegalStateException("required video track missing")
            val videoDuration=vfmt.getLong(MediaFormat.KEY_DURATION,0L)
            ...
            var audioFormat:MediaFormat?=null;var audioTrack=-1
            for(i in 0 until ae.trackCount){val f=ae.getTrackFormat(i);if(f.getString(MediaFormat.KEY_MIME)?.startsWith("audio/")==true){audioFormat=f;audioTrack=i;break}}
            val afmt=audioFormat?:throw IllegalStateException("AAC audio track missing after encoding")
            val amime=afmt.getString(MediaFormat.KEY_MIME)?:throw IllegalStateException("AAC mime missing")
            if(!amime.startsWith("audio/"))throw IllegalStateException("invalid encoded audio mime: $amime")

            mux=MediaMuxer(out.absolutePath,MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4)
            val vt=mux.addTrack(vfmt);val at=mux.addTrack(afmt)
            ve.selectTrack(videoTrack);ae.selectTrack(audioTrack)
            mux.start()
            copyTrackBounded(ve,mux,vt,videoDuration)
            copyTrackBounded(ae,mux,at,videoDuration)
            mux.stop()
            mux.release();mux=null
        }catch(e:Exception){
            ...
            try{vf.close()}catch(_:Exception){}
    ...
    private fun copyTrackBounded(ex:MediaExtractor,mux:MediaMuxer,track:Int,maxDurationUs:Long){
        ...
        while(true){
            ...
            val pts=ex.sampleTime;if(pts<0)break
        ...
        try{
            ...
            while(raf.filePointer+8L<=raf.length()){
                ...
                when(String(id,Charsets.US_ASCII)){
                    ...
                    "data"->{dataOffset=raf.filePointer;dataSize=size;raf.seek((dataOffset+size).coerceAtMost(raf.length()))}
                }
                val next=chunkStart+size+(size and 1L);if(next<=raf.length())raf.seek(next)
    ...
    private fun readLeShort(raf:java.io.RandomAccessFile):Int{
        val a=raf.read();val b=raf.read();if(a<0||b<0)throw IllegalStateException("truncated WAV")
        return a or (b shl 8)
    }
    private fun readLeInt(raf:java.io.RandomAccessFile):Int{
        val a=raf.read();val b=raf.read();val c=raf.read();val d=raf.read();if(a<0||b<0||c<0||d<0)throw IllegalStateException("truncated WAV")
    ...
    private fun encodeWavToAacMp4(wav:File,out:File){
        ...
        val fmt=MediaFormat.createAudioFormat("audio/mp4a-latm",w.sampleRate,w.channels).apply{
        ...
        try{
            ...
            while(!done){
                if(!eosQueued){
                    ...
                    if(inIndex>=0){
                        val input=codec.getInputBuffer(inIndex)?:throw IllegalStateException("AAC input buffer unavailable");input.clear()
                        val frameBytes=w.channels*2;var toRead=minOf(input.remaining(),remaining.toInt());toRead-=toRead%frameBytes
                        if(toRead>0){val bytes=ByteArray(toRead);raf.readFully(bytes);input.put(bytes);val frames=toRead/frameBytes;codec.queueInputBuffer(inIndex,0,toRead,ptsUs,0);ptsUs+=(frames*1000000L)/w.sampleRate;remaining-=toRead}
                        else{codec.queueInputBuffer(inIndex,0,0,ptsUs,MediaCodec.BUFFER_FLAG_END_OF_STREAM);eosQueued=true}
                    }
                }
                when(val outIndex=codec.dequeueOutputBuffer(MediaCodec.BufferInfo(),10000)){
                    MediaCodec.INFO_OUTPUT_FORMAT_CHANGED->{
                        if(muxStarted)throw IllegalStateException("AAC output format changed twice")
                        mux=MediaMuxer(out.absolutePath,MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4)
                        muxTrack=mux.addTrack(codec.outputFormat);mux.start();muxStarted=true
                    }
                    MediaCodec.INFO_TRY_AGAIN_LATER->{}
                        ...
                        if(outIndex>=0){
                            ...
                            if(!muxStarted)throw IllegalStateException("AAC muxer not started")
                            // Output buffers are fetched below with a second dequeue to retain BufferInfo.
                        }
                    }
                }
                // Drain output with retained BufferInfo in a dedicated pass.
                while(true){
                    val info=MediaCodec.BufferInfo();val oi=codec.dequeueOutputBuffer(info,0)
                    if(oi==MediaCodec.INFO_OUTPUT_FORMAT_CHANGED){
                        if(muxStarted)continue
                        mux=MediaMuxer(out.absolutePath,MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);muxTrack=mux.addTrack(codec.outputFormat);mux.start();muxStarted=true;continue
                    }
                    if(oi<0)break
                    val buffer=codec.getOutputBuffer(oi)
                    if(buffer!=null&&info.size>0&&(info.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG)==0){
                        if(!muxStarted)throw IllegalStateException("AAC muxer not started")
                        buffer.position(info.offset);buffer.limit(info.offset+info.size);mux!!.writeSampleData(muxTrack,buffer,info)
                    }
            ...
            try{mux?.release()}catch(_:Exception){}
            if(!out.isFile||out.length()<1024L){if(out.exists())out.delete();throw IllegalStateException("AAC encoding produced no usable file")}
        }
    }
'''

if old not in a:
    raise SystemExit('STAGE 11 HARDENING: mux implementation target not found')
a = a.replace(old,new,1)

old_assemble = '''        try{muxVideoAudio(video,File(audioPath),out);if(!out.isFile||out.length()<1024){fail(11,"Assembly produced no usable MP4");return}
...
new_assemble = '''        try{muxVideoAudio(video,File(audioPath),out);if(!out.isFile||out.length()<1024){fail(11,"Assembly produced no usable MP4");return}
...
if old_assemble not in a:
    raise SystemExit('STAGE 11 HARDENING: assemble gate target not found')
a = a.replace(old_assemble,new_assemble,1)
activity.write_text(a,encoding='utf-8')
print('STAGE 11 HARDENING: PASS')

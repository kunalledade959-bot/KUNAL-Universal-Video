...
3) fn='selectTarget'; label='Target APK Selection'; checks=("targetSpinner.selectedItemPosition" "putString(TARGET," "UniversalAccessibilityService.targetPackage = target");;
4) fn='studyTarget'; label='Study Selected APK'; checks=("getApplicationInfo(target," "getLaunchIntentForPackage(target)" "launch");;
5) fn='saveStory'; label='Story Input'; checks=("s.length < 10" "putString(STORY," "pass(5,");;
6) fn='operateTarget'; label='Operate Selected Target APK'; checks=("getLaunchIntentForPackage(target)" "startActivity(i)" "pass(6," "waitForTargetForeground(0)" "targetForeground");;
7) fn='deepStudy'; label='Deep Target-App Understanding'; checks=("rootInActiveWindow" "AccessibilityNodeInfo" "target_ui_map" "pass(7,");;
8) fn='scenePlan'; label='Exact Scene Plan'; checks=("SCENE_" "ACTION=" "putString(SCENES," "pass(8,");;
9) fn='buildPlan'; label='Production Plan / Prompts'; checks=("VISUAL_PROMPT=" "ACTION_PROMPT=" "putString(PLAN," "pass(9,");;
...
11) fn='assembleEdit'; label='Assemble / Edit'; checks=("MediaMuxer" "muxVideoAudio" "latestRecording()" "putString(FINAL,");;

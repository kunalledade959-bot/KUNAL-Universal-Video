...
class StageGate(context: Context) {
    ...
    private val stages = arrayOf(
        ...
        "Study Selected APK", "Story Input", "Operate Selected Target APK", "Deep Target-App Understanding",
        "Exact Scene Plan", "Production Plan / Prompts", "Audio / Voice / Music / Sound Effects",
    ...
    @Synchronized fun pass(id: Int, evidence: String): Boolean {
        ...
        stages[id - 1].evidence = evidence.trim()
        if (id < 13) {
    ...
    @Synchronized fun fail(id: Int, evidence: String) {
        ...
        stages[id - 1].evidence = evidence.trim().ifBlank { "Stage failed without evidence" }
    ...
    @Synchronized fun resetForRepair(id: Int): Boolean {
        ...
        stages[id - 1].evidence = "Repair requested; stage must re-execute its real verification"
        persist()
        return true
    }
    ...
    /** Synchronous persistence is deliberate: a crash immediately after a transition must not erase evidence. */
    private fun persist() {
        val committed = prefs.edit().putString("state", evidenceJson().toString()).commit()
        if (!committed) throw IllegalStateException("STAGE_GATE_PERSISTENCE_FAILED")
    }
    ...
    private fun restore() {
        ...
        try {
            ...
            for (i in 0 until 13) {
                ...
                if (st == State.PASS && (!previousPassed || st == State.PASS && stages[i].evidence.isBlank())) throw IllegalStateException("invalid_pass_chain")
                ...
                if (st == State.RUNNING) {
                    ...
                    stages[i].evidence = "Interrupted while RUNNING; repair required before retry"
                    for (n in i + 1 until 13) { stages[n].state = State.LOCKED; stages[n].evidence = "" }
                    persist()
                    break
                }
                if (st == State.FAIL) {

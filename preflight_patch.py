...
if activity.is_file():
    ...
    if old_connect not in a: raise SystemExit('REAL CONNECT FIX: target not found')
    ...
    if old_operate not in a: raise SystemExit('REAL OPERATE FIX: target not found')
...
else: raise SystemExit('UI FIX: activity_fixed.kt missing')

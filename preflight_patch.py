...
if activity.is_file():
    ...
    if old_end not in a: raise SystemExit('UI FIX: actions layout target not found')
    ...
    if old_order not in a: raise SystemExit('STAGE ORDER FIX: Stage 2 target dependency not found')
    ...
    if old_stage3 not in a: raise SystemExit('STAGE ORDER FIX: Stage 3 target connection point not found')
    a=a.replace(old_stage3,new_stage3,1)

    # Stage 10 completion: only PASS after STOP has produced a NEW finalized recording.
    ...
    if old_record not in a: raise SystemExit('STAGE 10 FIX: audio start target not found')
    ...
    if old_stop not in a: raise SystemExit('STAGE 10 FIX: recording stop target not found')
    a=a.replace(old_stop,new_stop,1)
    old_query='''"${MediaStore.Video.Media.DISPLAY_NAME} LIKE ?",arrayOf("KunalUniversalVideo_%"),"${MediaStore.Video.Media.DATE_ADDED} DESC"'''
    new_query='''"${MediaStore.Video.Media.DISPLAY_NAME} LIKE ? AND ${MediaStore.Video.Media.DATE_ADDED} >= ?",arrayOf("KunalUniversalVideo_%",(prefs().getLong("recording_start",0L)).toString()),"${MediaStore.Video.Media.DATE_ADDED} DESC"'''
    if old_query not in a: raise SystemExit('STAGE 10 FIX: recording freshness query not found')
    a=a.replace(old_query,new_query,1)
    activity.write_text(a,encoding='utf-8')
    print('UI + STAGE ORDER + STAGE 10 FRESHNESS FIX: PASS')
else:

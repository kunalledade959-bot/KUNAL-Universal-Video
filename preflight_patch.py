...
if activity.is_file():
    ...
    if old_story in a:a=a.replace(old_story,new_story,1)
    ...
    if old_end in a:a=a.replace(old_end,new_end,1)
    ...
    if old_connect in a:a=a.replace(old_connect,new_connect,1)
    ...
    if old_operate in a:a=a.replace(old_operate,new_operate,1)
    ...
    if old_stop in a:a=a.replace(old_stop,new_stop,1)
        ...
        old_stop2='fun stopRecordingFromBridge(){startService(Intent(this@MainActivity,ScreenCaptureService::class.java).setAction(ScreenCaptureService.STOP))}'
        if old_stop2 in a:a=a.replace(old_stop2,new_stop,1)
...
if old_bridge in s:s=s.replace(old_bridge,new_bridge,1)

...
s=s.replace('if static_ok:build_ok,bi=build()', 'if static_ok:build_ok,bi=build(); log("BUILD_RESULT="+json.dumps(bi,indent=2)); log("BUILD_LOG_TAIL="+BUILD_LOG.read_text(encoding="utf-8",errors="replace")[-16000:] if BUILD_LOG.exists() else "BUILD_LOG_MISSING")')

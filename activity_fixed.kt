...
class MainActivity : androidx.activity.ComponentActivity() {
    companion object {
        ...
        const val RECORDING = "recording_uri"
        const val FINAL = "final_uri"
    ...
    private val capture = registerForActivityResult(
        ...
        if (r.resultCode == Activity.RESULT_OK && r.data != null) {
            val i = Intent(this, ScreenCaptureService::class.java)
                ...
                .putExtra(ScreenCaptureService.CODE, r.resultCode)
                .putExtra(ScreenCaptureService.DATA, r.data)
            if (android.os.Build.VERSION.SDK_INT >= 26) startForegroundService(i) else startService(i)
            status.text = "Stage 10 RUNNING • recording started"
        } else {
            fail(10, "Screen-recording permission cancelled")
        }
    }
    ...
    private fun buildUi(p: android.content.SharedPreferences) {
        ...
        root.addView(TextView(this).apply {
            text = "Kunal Universal Video • 13 Stage Engine"
            textSize = 25f
        })
        ...
        story = EditText(this).apply {
            hint = "5 • Enter story"
            minLines = 4
            setText(p.getString(STORY, ""))
        }
        root.addView(story, LinearLayout.LayoutParams(-1, 180))
        val actions = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
        fun button(t: String, c: () -> Unit) = Button(this).apply {
            text = t
    ...
    private fun begin(id: Int): Boolean {
        if (!gate.isUnlocked(id) || !gate.begin(id)) {
            status.text = "Stage $id LOCKED or already active"
            return false
        }
        return true
    }

    private fun pass(id: Int, e: String) {
        if (gate.pass(id, e)) {
            status.text = "Stage $id PASS • $e"
            renderStatus()
        } else {
            status.text = "Stage $id PASS REJECTED by gate"
        }
    }

    private fun fail(id: Int, e: String) {
        gate.fail(id, e)
        status.text = "Stage $id FAIL • $e"
        renderStatus()
    }
    ...
    private fun stage1() {
        if (gate.state(1) != StageGate.State.PASS) {
            ...
            if (gate.begin(1)) gate.pass(1, "MainActivity launched; workflow controller attached")
        }
        loadApps()
        renderStatus()
    }
    ...
    private fun renderStatus() {
        ...
        val st = gate.state(id)
        status.text = "Stage $id • ${st.name} • ${gate.evidenceJson().optBoolean("final_pass")}\nSession ${sid.take(8)}"
    }
    ...
    private fun selectTarget() {
        ...
        if (pos !in apps.indices) {
            fail(3, "No target selected")
            return
        }
    ...
    private fun studyTarget() {
        ...
        if (ai == null) {
            fail(4, "Target package not installed")
            return
        }
        val launch = packageManager.getLaunchIntentForPackage(target) != null
        if (!launch) {
    ...
    private fun saveStory() {
        if (!begin(5)) return
        val s = story.text.toString().trim()
        if (s.length < 10) {
            fail(5, "Story must contain at least 10 characters")
            return
        }
        prefs().edit().putString(STORY, s).apply()
        pass(5, "Story persisted (${s.length} chars)")
    }

    private fun operateTarget() {
        if (!begin(6)) return
        val i = packageManager.getLaunchIntentForPackage(target)
        if (i == null) {
            fail(6, "Target cannot be launched")
            return
        }
    ...
    private fun deepStudy() {
        if (!begin(7)) return
        if (!UniversalAccessibilityService.isEnabled) {
            fail(7, "Accessibility service unavailable")
            return
        }
        val root = UniversalAccessibilityService.instance?.rootInActiveWindow
        if (root == null) {
            fail(7, "Target accessibility tree unavailable; target must be foreground")
            return
        }
        var nodes = 0
        var clickable = 0
        ...
        root.recycle()
        if (nodes < 1) {
            fail(7, "No accessibility nodes discovered")
            return
        }
    ...
    private fun scenePlan() {
        ...
        if (s.isBlank()) {
            fail(8, "Story missing")
            return
        }
        val parts = s.split(Regex("(?<=[.!?])\\s+")).filter { it.isNotBlank() }.take(30)
        val chunks = if (parts.isEmpty()) listOf(s) else parts
        val scenes = chunks.mapIndexed { idx, x ->
            "SCENE_${idx + 1}\nACTION=${x.trim()}\nBACKGROUND=scene-specific\nCHARACTER=consistent\nCLIP=recorded\n"
        }.joinToString("\n")
        prefs().edit().putString(SCENES, scenes).apply()
        pass(8, "Ordered scene plan created: ${chunks.size} scenes")
    }
    ...
    private fun buildPlan() {
        ...
        pass(9, "Production prompts generated for every ordered scene")
    }

    private fun audioAndRecord() {
        if (!begin(10)) return
        val text = prefs().getString(STORY, "") ?: ""
        if (text.isBlank()) {
            fail(10, "Story missing for narration")
            return
        }
        val engine = tts
        if (engine == null) {
            fail(10, "Text-to-speech engine is not initialized")
            return
        }
        prefs().edit().remove(RECORDING).putLong(RECORDING_START, System.currentTimeMillis() / 1000L).apply()
        val out = File(cacheDir, "narration_${sid}.wav")
        try {
            ...
            engine.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                ...
                override fun onDone(utteranceId: String?) {
                    runOnUiThread {
                        if (gate.state(10) != StageGate.State.RUNNING) return@runOnUiThread
                        if (!out.isFile || out.length() < 100) {
                            fail(10, "TTS completion reported but narration file is unusable")
                ...
                override fun onError(utteranceId: String?) {
                    runOnUiThread {
                        if (gate.state(10) == StageGate.State.RUNNING) {
                            fail(10, "Text-to-speech reported an error")
                        }
                    }
                }
            })
            val result = engine.synthesizeToFile(text, Bundle(), out, "kunal_narration_${sid}")
            if (result != TextToSpeech.SUCCESS) fail(10, "TTS synthesis request was rejected")
        } catch (e: Exception) {
            fail(10, "Audio synthesis failed: ${e.javaClass.simpleName}")
        }
    }

    fun startRecordingFromBridge() {
        if (gate.state(10) == StageGate.State.READY) audioAndRecord()
    }
    ...
    private fun waitForRecordingCompletion(attempt: Int) {
        ...
        if (attempt >= 20) {
            fail(10, "Recording stop completed but no new finalized MediaStore video was found")
            return
        }
        mainHandler.postDelayed({ waitForRecordingCompletion(attempt + 1) }, 500L)
    }

    private fun latestRecording(): android.net.Uri? {
        prefs().getString(RECORDING, null)?.let {
            try { return android.net.Uri.parse(it) } catch (_: Exception) {}
        }
        val since = prefs().getLong(RECORDING_START, 0L)
        val c = contentResolver.query(
            MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
            arrayOf(MediaStore.Video.Media._ID),
            "${MediaStore.Video.Media.DISPLAY_NAME} LIKE ? AND ${MediaStore.Video.Media.DATE_ADDED} >= ?",
            arrayOf("KunalUniversalVideo_%", since.toString()),
            "${MediaStore.Video.Media.DATE_ADDED} DESC"
        ) ?: return null
        return c.use {
            if (it.moveToFirst()) {
                MediaStore.Video.Media.EXTERNAL_CONTENT_URI.buildUpon()
                    .appendPath(it.getString(0)).build()
            } else null
        }
    }
    ...
    private fun assembleEdit() {
        if (!begin(11)) return
        val video = latestRecording()
        val audioPath = prefs().getString(AUDIO, "") ?: ""
        if (video == null) {
            fail(11, "No recorded visual clip found")
            return
        }
        if (audioPath.isBlank() || !File(audioPath).isFile) {
            fail(11, "Generated narration audio missing")
            return
        }
        val out = File(cacheDir, "assembled_${sid}.mp4")
        try {
            muxVideoAudio(video, File(audioPath), out)
            if (!out.isFile || out.length() < 1024) {
                fail(11, "Assembly produced no usable MP4")
                return
            }
            val check = MediaExtractor()
            try {
                check.setDataSource(out.absolutePath)
                var hasVideo = false
                var hasAudio = false
                for (i in 0 until check.trackCount) {
                    val mime = check.getTrackFormat(i).getString(MediaFormat.KEY_MIME) ?: ""
                    hasVideo = hasVideo || mime.startsWith("video/")
                    hasAudio = hasAudio || mime.startsWith("audio/")
                }
                if (!hasVideo || !hasAudio) {
                    fail(11, "Assembly output missing required video/audio tracks")
                    return
                }
            } finally {
                check.release()
            }
            prefs().edit().putString(FINAL, out.absolutePath).apply()
            pass(11, "Production assembly verified: video + AAC narration + synchronized MP4")
        } catch (e: Exception) {
            if (out.exists()) out.delete()
            fail(11, "Assembly failed: ${e.javaClass.simpleName}: ${e.message}")
        }
    }
    ...
    private fun muxVideoAudio(videoUri: android.net.Uri, audio: File, out: File) {
        ...
        try {
            ...
            if (vt < 0 || at < 0) throw IllegalStateException("required media track missing")
            mux!!.start()
            copyTrack(ve, mux!!, vt)
            copyTrack(ae, mux!!, at)
            mux!!.stop()
        ...
        if (!out.isFile || out.length() < 1024) throw IllegalStateException("assembled output invalid")
    }
    ...
    private fun copyTrack(ex: MediaExtractor, mux: MediaMuxer, track: Int) {
        ...
        while (true) {
            ...
            if (pts < 0L || pts < last) throw IllegalStateException("non-monotonic media timestamps")
            last = pts
            info.offset = 0
            info.size = n
    ...
    private fun verifyAndFix() {
        if (!begin(12)) return
        val p = prefs().getString(FINAL, "") ?: ""
        val f = if (p.isNotBlank()) File(p) else null
        if (f == null || !f.isFile || f.length() < 1024) {
            fail(12, "Assembled MP4 missing or unreadable")
            return
        }
        ...
        try {
            ...
            if (!hasVideo || !hasAudio) {
                fail(12, "MP4 media-track verification failed: video=$hasVideo audio=$hasAudio")
                return
            }
            if (durationUs <= 0L) {
                fail(12, "MP4 duration is not positive")
                return
            }
            pass(12, "Verified MP4: video track + audio track + duration=${durationUs}us")
        } catch (e: Exception) {
            fail(12, "Verification failed: ${e.javaClass.simpleName}: ${e.message}")
        } finally {
            extractor.release()
        }
    }
    ...
    private fun finalExport() {
        if (!begin(13)) return
        val p = prefs().getString(FINAL, "") ?: ""
        val src = if (p.isNotBlank()) File(p) else null
        if (src == null || !src.isFile || src.length() < 1024) {
            fail(13, "Verified assembled MP4 missing")
            return
        }
        try {
            val name = "KunalUniversalVideo_${System.currentTimeMillis()}.mp4"
            val v = ContentValues().apply {
                put(MediaStore.Video.Media.DISPLAY_NAME, name)
                put(MediaStore.Video.Media.MIME_TYPE, "video/mp4")
                if (android.os.Build.VERSION.SDK_INT >= 29) {
                    put(MediaStore.Video.Media.RELATIVE_PATH, "Movies/KunalUniversalVideo")
            ...
            try {
                contentResolver.openOutputStream(uri).use { os ->
                    FileInputStream(src).use { input -> input.copyTo(os!!) }
                }
            } catch (e: Exception) {
            ...
            prefs().edit().putString(FINAL, uri.toString()).apply()
            pass(13, "Final MP4 exported to Gallery/Movies/KunalUniversalVideo")
        } catch (e: Exception) {
            fail(13, "Gallery export failed: ${e.javaClass.simpleName}: ${e.message}")
        }
    }

...
/** Startup UI with fail-closed StageGate enforcement. */
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        gate = StageGate(this)
        if (gate.state(1) == StageGate.State.LOCKED || gate.state(1) == StageGate.State.READY) gate.begin(1)
        if (gate.state(1) == StageGate.State.RUNNING) gate.pass(1, "MainActivity created and UI attached")
        renderGateStatus()
        ...
        spinner.onItemSelectedListener = object : android.widget.AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: android.view.View?, position: Int, id: Long) {
                if (position in apps.indices) {
                    ...
                    if (gate.begin(3)) gate.pass(3, "Installed target selected: $target")
                    renderGateStatus()
                }
            }
        }
        findViewById<Button>(R.id.connect).setOnClickListener {
            if (!gate.isUnlocked(2)) { status.text = "Stage 2 LOCKED: Stage 1 must PASS"; return@setOnClickListener }
            if (target.isBlank()) { status.text = "Select target APK"; return@setOnClickListener }
            if (!UniversalAccessibilityService.isEnabled) {
                gate.begin(2); gate.fail(2, "Accessibility service is not enabled")
                status.text = "Stage 2 FAIL: enable Accessibility permission"
                return@setOnClickListener
            }
            gate.begin(2); bridge?.connect(target); gate.pass(2, "Local bridge connected with Accessibility enabled")
            status.text = "Stage 2 PASS • REAL SESSION CONNECTED • ${sid.take(8)}"
            renderGateStatus()
        }
        ...
        findViewById<Button>(R.id.openTarget).setOnClickListener {
            if (!gate.isUnlocked(3) || target.isBlank()) { status.text = "Stage 3 LOCKED: select target APK"; return@setOnClickListener }
        ...
        findViewById<Button>(R.id.screenRecord).setOnClickListener {
            if (!gate.isUnlocked(2)) { status.text = "Recording LOCKED until Stage 2 PASS"; return@setOnClickListener }
            requestCapture()
        }
    ...
    override fun onResume() { super.onResume(); if (::gate.isInitialized) renderGateStatus() }
    private fun renderGateStatus() {
        val current = gate.currentStage()
        if (current <= 3 && gate.state(current) == StageGate.State.READY) status.text = "Stage $current READY • stages after it LOCKED"
    }
